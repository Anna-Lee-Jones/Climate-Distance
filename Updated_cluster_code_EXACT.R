...
#add empty distance column to for outputs
...
blank <- data_chunks[[1]]
blank <- blank[0,]
fwrite(blank, "OUTPUT_EXACT.csv", append = FALSE, row.names = FALSE, col.names = T, quote = FALSE)
...
foreach(chunk = data_chunks, .packages = c("data.table", "dplyr", "geosphere")) %dopar% {
  ...
  if (nrow(sample_match) > 0) {
    fwrite(sample_match, "OUTPUT_EXACT.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }
  ...
  for (n in seq_len(nrow(sample_na))) {
    ...
    # Find which cells within the remit are analogs at T2 to cell(n) at T1 in temp +-0.5 deg and rainfall +-5mm
  ...
  if (nrow(sample_na) > 0) {
    fwrite(sample_na, "OUTPUT_EXACT.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }
}
