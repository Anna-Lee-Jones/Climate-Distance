...
blank <- data_chunks[[1]]
blank <- blank[0,]
fwrite(blank, "OUTPUT_PLUSPLUS.csv", append = FALSE, row.names = FALSE, col.names = T, quote = FALSE)
...
foreach(chunk = data_chunks, .packages = c("data.table", "dplyr", "geosphere")) %dopar% {
  ...
  if (nrow(sample_match) > 0) {
    fwrite(sample_match, "OUTPUT_PLUSPLUS.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }  
  ...
  if (nrow(sample_na) > 0) {
    fwrite(sample_na, "OUTPUT_PLUSPLUS.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }
}
