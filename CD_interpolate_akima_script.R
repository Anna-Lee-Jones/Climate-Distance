...
start.time <- Sys.time()
val_interpol <- with(dt, akima::interp(X, Y, distance, xo = seq(min(-180), max(180), length =721),
                                       yo = seq(min(-90), max(90), length =361), duplicate="mean"))
beep(2)
end.time <- Sys.time()
...
d1 <- expand.grid(x = 1:721, y = 1:361) #same length as interpolation, positive values only
out <- transform(d1, z = val_interpol$z[as.matrix(d1)])#expand out interpolation Z matrix
out$x <- (out$x-361)/2#transform x and y coordinates back to lat long (-half length/factor of res)
out$y <- (out$y-181)/2
#save the interpolated data frame 
write.csv(out,"10mill_CD_interpolated_halfdegree.csv", row.names = TRUE)
