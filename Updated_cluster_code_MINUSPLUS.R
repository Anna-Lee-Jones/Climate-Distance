...
blank <- data_chunks[[1]]
blank <- blank[0,]
fwrite(blank, "OUTPUT_MINUSPLUS.csv", append = FALSE, row.names = FALSE, col.names = T, quote = FALSE)
...
foreach(chunk = data_chunks, .packages = c("data.table", "dplyr", "geosphere")) %dopar% {
  ...
  chunk$distance <- ifelse((chunk$T2_temp >= (chunk$T1_temp-0.5) &
                              chunk$T2_temp<=(chunk$T1_temp) &
                              chunk$T2_rain >= (chunk$T1_rain)&
                              chunk$T2_rain <= (chunk$T1_rain+5)), 0, NA)
  ...
  if (nrow(sample_match) > 0) {
    fwrite(sample_match, "OUTPUT_MINUSPLUS.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }  
  ...
  for (n in seq_len(nrow(sample_na))) {
    ...
    analogs <- remit[remit$T2_temp >= (sample_na[n,T1_temp-0.5])&
                       remit$T2_temp <= (sample_na[n,T1_temp])&
  ...
  if (nrow(sample_na) > 0) {
    fwrite(sample_na, "OUTPUT_MINUSPLUS.csv", append = TRUE, row.names = FALSE, col.names = FALSE, quote = FALSE)
  }
}
